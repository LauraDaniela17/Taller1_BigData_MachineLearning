...
Revisión inicial de la estructuración de los datos; cantidad de observaciones, tipos de datos, nombre de variables, identificación de valores faltantes (NA).
...
```{r}
# Se seleccionan de data_filter_inicial las variables independiente seleccionadas para el análisis
...
```{r}
# Revisar nuevamente la estructura de los datos
str(data_filter)
```
...
```
Imputar el valor faltante de Maximo nivel de educacion
```{r}
...
vars <- data_filter[, c("ingreso_hora_1", "age", "hours_work_usual", "max_educ_level", "sex_label")]
...
```{r}
ggplot(datos_lim, aes(x = max_educ_level, y = log_ingreso_hora)) +
...
# Gráfico
ggplot(datos_lim, aes(x = factor(formal, levels = c(0, 1), labels = c("No formal", "Formal")))) +
...
boot_peak <- replicate(B, {
  ...
  bb  <- coef(fit)
  if (is.na(bb["age2"]) || abs(bb["age2"]) < 1e-10) return(NA_real_)
  - bb["age"] / (2*bb["age2"])
})
...
ci_perc #Intervalo de confianza
...
Exportar tabla
```{r}
kbl(tabla_peak, format = "latex", booktabs = TRUE,
    caption = "Edad pico del perfil edad–salario (OLS, bootstrap percentil)") |>
  kable_styling(latex_options = c("hold_position")) |>
  save_kable("tabla_edad_pico_bonita.tex")
...
a<- ggplot(datos_lim, aes(x = ingreso_hora_1 )) +
  geom_histogram(mapping = aes(y = (after_stat(count))/sum(after_stat(count))), 
                 color ="#FFFFFF", fill= "sandybrown", show.legend = FALSE, na.rm = TRUE, 
  ...
  geom_vline(xintercept = median(datos_lim$ingreso_hora_1, na.rm = TRUE), color = 'darkturquoise', linetype = 'solid') +
  geom_vline(xintercept = mean(datos_lim$ingreso_hora_1, na.rm = TRUE), color = 'red', linetype = 'dashed') +
  labs(x = "ingreso_hora_1", y = "Percent (%)") +
  theme_classic() 
a
```
...
b <- ggplot(datos_lim, aes(x = log_ingreso_hora)) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)), 
                     labels = scales::label_number(scale = 100)) +
  scale_x_continuous(expand = expansion(mult = c(0, 0)), limits = c(0, NA)) +
  geom_vline(xintercept = median(datos_lim$log_ingreso_hora, na.rm = TRUE), color = 'darkturquoise', linetype = 'solid') +
...
```
como se observa en los histogramas anteriores la distribucion de ingreso por hora tiene muy marcada la cola a la derecha, por esta razon se elegirá el logaritmo del ingreso por hora.
...
```{r}
set.seed(10101)  # Semilla para propositos de replicabilidad


inTrain <- createDataPartition(
  y = datos_lim$log_ingreso_hora,  #especificar los datos requeridos
  p = .70, # Porcentaje muestra de entrenamiento
  list = FALSE
)
training <- datos_lim |> filter(row_number() %in% inTrain)
testing  <- datos_lim |> filter(!(row_number() %in% inTrain))

# Crear datos para visualizacion
...
# Visualizacion de las muestras de entrenamiento y prueba
ggplot(split_data, aes(x = Split, y = Count)) +
  ...
  labs(title = "Distribución de la muestra de entrenamiento y de prueba",
       ...
       x = "") +
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) + #Escala de las etiquetas
  theme_classic()
```
...
```
 - VEA - MODELO 6
```{r}
form_6 <- log_ingreso_hora ~ sex + relab + formal + size_firm
modelo6a <- lm(form_6,
               data = training )
...
```
 ...
 - VSA - MODELO 8
```{r}
form_8 <- log_ingreso_hora ~ max_educ_level * relab + formal + age + sex
modelo8a <- lm(form_8,
               data = training )
...
tabla_rmse <- data.frame(
  ...
  Especificación_Modelo = c(
    ...
    "Sexo * Educación + Edad",
    "Sexo + Tipo Ocupación + Formal + Tamaño empresa",
    "Edad + Educación * Formal",
    "Educación * Tipo Ocupación + Formal + Edad + Sexo"
  ),
...
tabla_flex <- flextable(tabla_rmse)
...
doc <- read_docx() |> 
  body_add_par("Resumen de Modelos Predictivos con RMSE", style = "heading 1") |> 
  body_add_flextable(tabla_flex)

# Guardar
print(doc, target = "Resumen_Modelos_RMSE.docx")
...
modelo3_loocv <- train(
  form_3,
  data = datos_lim,   # Usa todo el dataset
  method = "lm",
  trControl = ctrl_loocv
)

# Obtener el RMSE
modelo3_loocv$results$RMSE
