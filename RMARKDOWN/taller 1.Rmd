...
ResultadosRMSE<- data.frame( Modelo = c(1, 2),
...
```{r}
## let's change the way of saving the data.
...
scores3 <- data.frame( Modelo = rep(c(1, 2),3),
                     Approach= c(rep("VSA",4),rep("K-Fold",4),rep("LOOCV",4)),
                     RMSE= c(RMSE_vsa2, RMSE_kfold2, RMSE_loocv2)
                    )

ggplot(scores3, ) + 
  geom_line(aes(x=Modelo,y=RMSE,col=Approach), size=0.5)+
  theme_bw() 
```
