...
# Generación de URL: contiene un vector de 10 paginaciones
...
cat("El dataset filtrado contiene", nrow(data_filter), "filas y", ncol(data_filter), "columnas.\n")
...
data_filter<- data_filter%>% select(directorio, secuencia_p, orden,sex, age, relab, mes,
                                        formal, size_firm, max_educ_level, impa, hours_work_usual, fex_c)
...
datos_lim <- data_filter[!is.na(data_filter$log_ingreso_hora), ]

ggplot(datos_lim, aes(x = log_ingreso_hora, fill = sex_label)) +
...
diseno <- svydesign(ids = ~1, weights = ~fex_c, data = datos_lim)

age_wage_reg_pond <- svyglm(log_ingreso_hora ~ age + age2, design = diseno)
summary(age_wage_reg_pond)

msummary(
  list("Perfil edad–salario (ponderado)" = age_wage_reg),
  stars = c('*' = .10, '**' = .05, '***' = .01),
  statistic = "({std.error})",
  ...
  notes = "Errores estándar robustos por diseño de encuesta. Ponderación: fex_c.",
  output = "tabla_regresion_ponderada.tex"
)
...
gap_incond <- 100*(exp(coef(wage_gap)["sex_label"]) - 1)
gap_incond
...
# Interpretación en porcentaje
gap_cond <- 100*(exp(coef(gap_cond)["sex_label"]) - 1)
gap_cond
