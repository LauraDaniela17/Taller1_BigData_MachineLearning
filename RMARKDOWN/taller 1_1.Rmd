...
$$
\ text{sex} = variable \ binaria \ de \ sexo
$$

$$
X = \{ \text{edad}, \text{edad}^2, \text{educación}, \text{oficio}, \text{formalidad}, \text{tamaño de empresa}, \text{horas} \}
$$
$$ 
...
```{r}
# Filtro de datos
gap_cond <- lm(log(ingreso_hora_1) ~ sex + age + I(age^2) + max_educ_level +
...
# Paso 2: ln(w) ~ X
res_w <- lm(log(ingreso_hora_1) ~ age + I(age^2) + max_educ_level +
               oficio + formal + size_firm + hours_work_usual, 
            data = datos_lim)$residuals
...
boot_fwl <- function(data, index){
  ...
  r_sex <- lm(sex ~ age + I(age^2) + max_educ_level +
                 oficio + formal + size_firm + hours_work_usual, 
              data = d)$residuals
  r_w   <- lm(log(ingreso_hora_1) ~ age + I(age^2) + max_educ_level +
                 oficio + formal + size_firm + hours_work_usual, 
              data = d)$residuals
  coef(lm(r_w ~ r_sex))[2]
}
...
Instalar librerias necesarias para predicción de Salario por hora según las variables seleccionadas.
...
Analisis de la tabla utilizada para verificar los modelos de prediccion de salario por hora
